...
class TestBudgetExtraction:
    ...
    def test_dollar_amount_extraction(self):
        ...
        result = _extract_budget_info("I have a budget of $1000")
        assert "budget_amount" in result
        assert result["budget_amount"]["max"] == 1000
        assert result["budget_amount"]["currency"] == "USD"
        
        # Test comma-separated amounts
        result = _extract_budget_info("My budget is $1,500")
        assert "budget_amount" in result
        assert result["budget_amount"]["max"] == 1500
        
        # Test decimal amounts
        result = _extract_budget_info("Budget: $2,500.00")
        assert "budget_amount" in result
        assert result["budget_amount"]["max"] == 2500
    
    def test_text_amount_extraction(self):
        """Test extraction of text-based amounts."""
        # Test "dollars" format
        result = _extract_budget_info("I have 1000 dollars")
        assert "budget_amount" in result
        assert result["budget_amount"]["max"] == 1000
        
        # Test "USD" format
        result = _extract_budget_info("Budget is 1500 USD")
        assert "budget_amount" in result
        assert result["budget_amount"]["max"] == 1500
    ...
    def test_case_insensitive_extraction(self):
        ...
        result = _extract_budget_info("BUDGET of $1000")
        assert "budget_amount" in result
        assert result["budget_amount"]["max"] == 1000
        assert "budget_range" in result
        assert result["budget_range"] == "budget"
        
        result = _extract_budget_info("LUXURY accommodations")
        assert result["budget_range"] == "luxury"
...
class TestEdgeCases:
    ...
    def test_none_values(self):
        ...
        assert _extract_budget_info(None) is None
        assert _extract_date_info(None) is None
...
class TestIntegration:
    ...
    def test_complete_travel_request(self):
        ...
        assert "budget_amount" in budget_result
        assert budget_result["budget_amount"]["max"] == 2000
    ...
    def test_realistic_conversation_text(self):
        ...
        assert "budget_amount" in budget_result
        assert budget_result["budget_amount"]["max"] == 3000
